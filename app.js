...
async function runSetupChecks() {
  ...
  updateCheckItem(
    ...
    "warning",
    "⚠️",
    `Add ${currentUrl} to Supabase redirect URLs`
  );
  ...
  try {
    ...
    for (const talk of Object.values(talks)) {
      html += `<div class="result-card">
                <div class="result-title"><a href="${escapeHtml(
                  talk.url
                )}" target="_blank" class="result-title-link">${escapeHtml(
        talk.title
      )}</a></div>
                <div class="result-speaker">by ${escapeHtml(talk.speaker)}</div>
                <div class="result-sentences">${talk.sentences
                  .slice(0, 3)
                  .join("<br>")}</div>
            </div>`;
    }
    ...
    showResults(
      "keyword",
      `<div class="result-error">Error: ${escapeHtml(error.message)}</div>`
    );
  } finally {
    showLoading(false);
  }
...
async function semanticSearch() {
  ...
  try {
    ...
    if (!results || results.length === 0) {
      showResults(
        "semantic",
        '<div class="no-results">No similar content found. Try a different query.</div>'
      );
      return;
    }
    ...
    console.log(
      "Step 3: Deduplicated to",
      uniqueResults.length,
      "unique results"
    );

    // Step 4: Display results with similarity badges and similarity explorer
    let html = "";
    for (const result of uniqueResults) {
      const sim = result.similarity;
      const badge = similarityBadge(sim);
      const talkId = result.talk_id;
      html += `<div class="result-card" data-talk-id="${escapeHtml(talkId)}">
        ...
        result.title
                        ...
                        <div class="result-speaker">by ${escapeHtml(
                          result.speaker
                        )}</div>
                    </div>
                    ${badge}
                ...
                <div class="similar-talks-control">
                    <button class="btn-similar-talks" onclick="findSimilarTalks('${talkId}', '${escapeHtml(
        result.title
      )}', 'semantic')">
                        🔍 Find Similar Talks
                    </button>
                    <div class="similar-talks-results" id="similar-talks-${talkId}"></div>
                </div>
            </div>`;
    }
...
async function askQuestion() {
  ...
  try {
    ...
    for (const talk of topTalks) {
      ...
      html += `<div class="result-card result-source" data-talk-id="${escapeHtml(
        talkId
      )}">
                <div class="result-card-header">
                    <div>
...
async function searchSentences(embedding) {
  ...
  console.log(
    "Calling match_sentences RPC with embedding length:",
    embedding?.length
  );
...
function groupByTalk(sentences) {
  ...
  return Object.values(talkMap)
    .sort(
      (a, b) =>
        b.totalSimilarity / b.sentences.length -
        a.totalSimilarity / a.sentences.length
    )
    .slice(0, 3)
    .map((talk) => ({
...
async function fetchFullTalkText(talks) {
  ...
  if (error || !data) {
    console.warn(
      "Failed to fetch full talk text, using snippets:",
      error?.message
    );
    return talks;
  }
...
// ============================================
// TALK SIMILARITY EXPLORER
// ============================================

/**
 * Find similar talks to a given talk
 * @param {string} talkId - The UUID of the talk to find similar talks for
 * @param {string} talkTitle - The title of the talk (for display)
 * @param {string} sourceType - The type of search that triggered this ('semantic' or 'rag')
 */
async function findSimilarTalks(talkId, talkTitle, sourceType) {
  const resultsContainer = document.getElementById(`similar-talks-${talkId}`);
  ...
  resultsContainer.innerHTML =
    '<div class="similar-talks-loading">Finding similar talks...</div>';
  resultsContainer.style.display = "block";
  resultsContainer.dataset.loaded = "loading";

  try {
    // Step 1: Get a representative sentence from this talk to use as query
    // We'll get the first sentence to use as a representative
    const { data: talkSentences, error: fetchError } = await supabaseClient
      .from("sentence_embeddings")
      .select("text, title, speaker, url")
      .eq("talk_id", talkId)
      .not("embedding", "is", null)
      .limit(1)
      .single();
    ...
    let queryText;
    if (fetchError || !talkSentences) {
      // Fallback: Use the talk title to generate an embedding
      console.log("No sentences found, using talk title for embedding");
      queryText = talkTitle;
    } else {
      // Use the first sentence as a representative
      queryText = talkSentences.text;
    }

    // Step 2: Generate embedding for the query text
    const embedding = await getEmbedding(queryText);

    // Step 3: Find similar sentences (excluding the original talk)
    const similarResults = await searchSentences(embedding);

    // Step 4: Filter out the original talk and group by talk
    const filteredResults = similarResults.filter((r) => r.talk_id !== talkId);
    const similarTalks = groupSimilarTalks(filteredResults);

    // Step 5: Display results
    displaySimilarTalks(resultsContainer, similarTalks, talkTitle);
  } catch (error) {
    console.error("Error finding similar talks:", error);
    resultsContainer.innerHTML = `<div class="similar-talks-error">Error: ${escapeHtml(
      error.message
    )}</div>`;
  } finally {
...
/**
 * Group similar search results by talk
 */
function groupSimilarTalks(results) {
  const talkMap = {};
  ...
  for (const result of results) {
    ...
    if (!talkMap[talkId]) {
      talkMap[talkId] = {
        talk_id: talkId,
        title: result.title || "Unknown Talk",
    ...
    talkMap[talkId].sentences.push(result.text);
    talkMap[talkId].similarities.push(result.similarity);
  }
  ...
  return Object.values(talkMap)
    .map((talk) => ({
      ...talk,
      avgSimilarity:
        talk.similarities.reduce((a, b) => a + b, 0) / talk.similarities.length,
      sampleText: talk.sentences[0], // Use first sentence as preview
    }))
    .sort((a, b) => b.avgSimilarity - a.avgSimilarity)
    .slice(0, 5); // Top 5 similar talks
}

/**
 * Display similar talks in the results container
 */
function displaySimilarTalks(container, similarTalks, originalTalkTitle) {
  if (similarTalks.length === 0) {
    container.innerHTML =
      '<div class="similar-talks-empty">No similar talks found.</div>';
    return;
  }

  let html = `<div class="similar-talks-header">Similar to "${escapeHtml(
    originalTalkTitle
  )}":</div>`;

  for (const talk of similarTalks) {
    const badge = similarityBadge(talk.avgSimilarity);
    html += `<div class="similar-talk-item">
            <div class="similar-talk-header">
                <div>
                    <div class="similar-talk-title">
                        <a href="${escapeHtml(
                          talk.url
                        )}" target="_blank" class="result-title-link">${escapeHtml(
      talk.title
                    ...
                    <div class="similar-talk-speaker">by ${escapeHtml(
                      talk.speaker
                    )}</div>
                </div>
                ${badge}
            </div>
            <div class="similar-talk-preview">${escapeHtml(
              talk.sampleText
            )}</div>
        </div>`;
  }
...
// Make findSimilarTalks available globally for onclick handlers
window.findSimilarTalks = findSimilarTalks;
